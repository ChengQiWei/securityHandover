 ...
 *
 */
public class TcTssUserPsFileSystem extends TcTssPsFileSystem {
	...
	protected TcTssKmKeyinfo[] enumRegisteredKeysImpl(TcTssUuid keyUuid) throws TcTssException
	{
		...
		if (keyUuid==null) 
		{
			...
			// But no details on how the array should map the hierarchy to an array is given.
			// So we just return all registered keys.
			// Additionally, the root key is not identified uniquely for the user storage
			// It could be USK1 or USK2.
			// This implementation actually does not enforce using these key UUIDs.
			// However, if of these keys is present, it will be first in the returned list,
