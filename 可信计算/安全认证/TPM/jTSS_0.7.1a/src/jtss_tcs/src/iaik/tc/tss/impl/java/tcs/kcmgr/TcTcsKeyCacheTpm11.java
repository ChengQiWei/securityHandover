...
public class TcTcsKeyCacheTpm11 extends TcTcsKeyCache {
	...
	public boolean swapOutKeyNotParent(long khTpmParent) throws TcTddlException, TcTpmException, TcTcsException
	{
		...
		// TPM 1.1 style
		Log.debug("swapping out key using 1.1 style");
		long khTpmToSave = TcTcsKeyHandleMgr.getInstance().getTpmKhLruNotParent(khTpmParent);
		TcTddl dest = TcTddl.getInstance();
		Object[] tpmOutData = TcTpmCmdDeprContext.TpmSaveKeyContext(dest, khTpmToSave);
		cachedKey = new TcTcsCachedKey(TcTcsCachedKey.CT_SAVE_KEY_CONTEXT, tpmOutData[1]);
	...
	protected long swapInFromCache(long khTcs) throws TcTddlException, TcTpmException, TcTcsException
	{
		...
		// TPM 1.1 style
		if (cachedKey.getKeyType() == TcTcsCachedKey.CT_SAVE_KEY_CONTEXT) {
			tpmOutData = TcTpmCmdDeprContext.TpmLoadKeyContext(dest, (TcBlobData) cachedKey.getKeyBlob());
